...
This file provides guidance to Qwen Coder when working with code in this repository.
...
11. **Translation Format**: Each translation should start on a new paragraph immediately following its corresponding original text paragraph
12. **Quality Control (MANDATORY)**: After completing the translation, you MUST perform a secondary review to verify that the original text and translation match properly. Check for:
...
All git commits should follow semantic commit message format for better clarity and consistency:
...
For any git commit, you must add Qwen as a co-author using the following command:

```bash
git commit --amend --no-edit --trailer="Co-authored-by: qwen <noreply@tongyi.lab>"
```